...
package com.loading {
	...
	[Event(name="securityError", 		type="com.events.LoaderEvent")]
/**
 ...
 */	
	public class XMLLoader extends DataLoader {
		...
		override protected function _calculateProgress():void { 
			...
			var estimate:int = int(this.vars.estimatedBytes);
			if (this.vars.integrateProgress == false) {
				// do nothing
			} else if (_loadingQueue != null && (int(this.vars.estimatedBytes) < _cachedBytesLoaded || _loadingQueue.auditedSize)) { //make sure that estimatedBytes is prioritized until the _loadingQueue has audited its size successfully!
			...
			} else if (int(this.vars.estimatedBytes) > _cachedBytesLoaded && (!_initted || (_loadingQueue != null && _loadingQueue.status <= LoaderStatus.COMPLETED && !_loadingQueue.auditedSize))) {
				_cachedBytesTotal = int(this.vars.estimatedBytes);
			}
...
//---- EVENT HANDLERS ------------------------------------------------------------------------------------
		...
		override protected function _receiveDataHandler(event:Event):void {
			...
			_loadingQueue = new LoaderMax({name:this.name + "_Queue", maxConnections:(int(this.vars.maxConnections) || 2), skipFailed:Boolean(this.vars.skipFailed != false), skipPaused:Boolean(this.vars.skipPaused != false)});
