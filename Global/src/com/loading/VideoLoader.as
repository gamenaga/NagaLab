...
package com.loading {
	...
	[Event(name="netStatus", 	type="com.events.LoaderEvent")]
/**
 ...
 * 		<li><strong> bgColor : int </strong> - When a <code>width</code> and <code>height</code> are defined, a rectangle will be drawn inside the <code>ContentDisplay</code> immediately in order to ease the development process. It is transparent by default, but you may define a <code>bgAlpha</code> if you prefer.</li>
 ...
 */	
	public class VideoLoader extends LoaderItem {
		...
		public function VideoLoader(urlOrRequest:*, vars:Object=null) {
			...
			_video.deblocking = int(this.vars.deblocking);
	...
	trace("hit cue point " + event.data.name + ", message: " + event.data.parameters.message); 
		...
		protected function _forceInit():void {
			if (_ns.bufferTime >= _duration) {
				_ns.bufferTime = int(_duration - 1);
			}
			_initted = true;
...
//---- EVENT HANDLERS ------------------------------------------------------------------------------------
		...
		protected function _statusHandler(event:NetStatusEvent):void {
			...
			if (code == "NetStream.Play.Stop") {
				...
				if (this.vars.repeat == -1 || int(this.vars.repeat) > _repeatCount) {
					_repeatCount++;
