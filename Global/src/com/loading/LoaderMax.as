...
package com.loading {
	...
	[Event(name="ioError", 				type="com.events.LoaderEvent")]
 ...
 */	
	public class LoaderMax extends LoaderCore {		
		...
		public function LoaderMax(vars:Object=null) {
			...
			this.maxConnections = ("maxConnections" in this.vars) ? int(this.vars.maxConnections) : 2;
...
//---- EVENT HANDLERS ------------------------------------------------------------------------------------
		...
		protected function _loadNext(event:Event=null):void {
			...
			if (_status == LoaderStatus.LOADING) {
				...
				var loader:LoaderCore, loaders:Array = _loaders.concat(), l:int = loaders.length, activeCount:int = 0; //use _loaders.concat() because in some rare situations, a loader's COMPLETE event might occur immediately and if autoDispose is true, the length of the array could change mid-loop causing a skip.
				_calculateProgress();
				for (var i:int = 0; i < l; i++) {
					loader = loaders[i];
		...
		protected function _prioritizeHandler(event:Event):void {
			...
			if (_status == LoaderStatus.LOADING && loader.status <= LoaderStatus.LOADING && !(loader in _activeLoaders)) {
				_cancelActiveLoaders();
				var prevMaxConnections:int = this.maxConnections;
