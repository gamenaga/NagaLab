 ...
 */	
	public class MP3Loader extends LoaderItem {
		...
		public function MP3Loader(urlOrRequest:*, vars:Object=null) {
			...
			this.initThreshold = ("initThreshold" in this.vars) ? int(this.vars.initThreshold) : 102400;
			_initSound();
		}
...
//---- EVENT HANDLERS ------------------------------------------------------------------------------------
		...
		protected function _soundCompleteHandler(event:Event):void {
			if (int(this.vars.repeat) > _repeatCount || int(this.vars.repeat) == -1) {
				_repeatCount++;
				_playSound(0);
			} else {
				_repeatCount = 0;
