 ...
 **/
	public class FlexBlitMask extends UIComponent {
		 ...
		 */
		public function FlexBlitMask(target:DisplayObject=null, x:Number=0, y:Number=0, width:Number=100, height:Number=100, smoothing:Boolean=false, autoUpdate:Boolean=true, fillColor:int=0x00000000, wrap:Boolean=false) {
			super();
			if (width < 0 || height < 0) {
		...
		protected function _render(xOffset:Number=0, yOffset:Number=0, clear:Boolean=true, limitRecursion:Boolean=false):void {
			...
			var maxRow:int = int(((y + _height) - _bounds.y) / _gridSize);
			if (maxRow >= _rows) {
				maxRow = _rows - 1;
			}
