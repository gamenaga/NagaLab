package naga.ui
{
	...
	public class WordPan extends Sprite
	{
...
//			trace(this.numChildren,this.getChildAt(0),pan_img.numChildren,pan_img.parent);
			removeChildren();
			pan = null;
//			trace(this.numChildren,pan_img.numChildren,pan_img.parent);
//			pan_img.removeChild(pan_title);
//			trace(this.numChildren,pan_img.numChildren,pan_img.parent);
			for (var i:int= 1 ; i < pan_img.numChildren; i++)
			{
				if(pan_img.getChildAt(i) is Bitmap)
				{
					(pan_img.getChildAt(i) as Bitmap).bitmapData.dispose();
				}
				else
				{
					(pan_img.getChildAt(i)as Global.icons).dispose();
				}
			}
			pan_img.removeChildren();
