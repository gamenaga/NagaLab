package naga.ui.bitmapFont
{
	...
	public class BitmapFont
	{
		...
		public function BitmapFont(bitmapData:BitmapData, config:XML)
		{
			...
			this.m_fontName = String(config.info.@face);
			
			var itemList:XMLList = config.chars.char;
			...
			for (var i:int = 0; i < count; i++)
			{
				...
				this.m_charMap[charItem.charID] = i;
//				trace("bf 57:",i,item.@id,count,item,charItem.offsetX);
			}
//			trace("bf 59:",charItem.charID,m_charItemList[0].charID);
		}
		 ...
		 */		
		public function pastePixels(targetBitmapData:BitmapData, targetPoint:Point, char:int):Boolean
		{
			if (!char)
			{
				return false;
		 ...
		 * @return
		 */		
		public function getCharWidth(char:int):int
		{
			var i:Object = this.m_charMap[char];
			if (i === null)
			{
...
//			trace("bf 119:",char,this.m_charItemList[i].rect.width);
		 ...
		 * @return
		 */		
		public function getCharHeight(char:int):int
		{
			var i:Object = this.m_charMap[char];
			if (i === null)
			{
		 ...
		 * @return 
		 */		
		public function getCharOffsetY(char:int):int
		{
			var i:Object = this.m_charMap[char];
			if (i === null)
			{
...
//			trace(bitmapFont.fontName,_fontMap[bitmapFont.fontName]);
		}
		...
		public static function getFont(fontName:String):BitmapFont
		{
			if (_fontMap)
			{
//				trace(fontName,_fontMap[fontName]);
...
//		trace("bf 240:",xml.@id);
		...
		this.offsetX = int(xml.@xoffset);
		this.offsetY = int(xml.@yoffset);
		this.advanceX = int(xml.@xadvance);
	}
